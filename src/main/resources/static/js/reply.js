	...
	function updateReplyList(data){ 
		...
		for(let r of data){
			...
			str += `<li id="${r.id}" class="list-group-item">`
                ...
                + '</span>'
                + `<button type="button" data-toggle="collapse" data-target=".updateReplyBox-${r.id}" class="bi bi-pencil-square">` + '수정' + '</button>'
                + `<button type="button" id="btnDelete" class="bi bi-x-square mx-1" data-rid="${r.id}">` + '삭제' + '</button> '
                + `<p class="collapse updateReplyBox-${r.id} show">` + r.replyText + '</p>'
